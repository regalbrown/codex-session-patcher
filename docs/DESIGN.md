...
Muggle Leads 管理后台提供广告位表单，作者登录后选择 tab 和左右位置，上传广告图片或填写已有图片 URL，再保存点击链接、尺寸和显示方式。广告元数据保存到 D1，上传图片保存到 R2；本地用户不需要也不能参与配置。对外配置使用 `slots` 数组描述广告位。每项包含 `tab`、`position`、`enabled`、`image_url`、`click_url`、`alt`、`title`、`width`、`max_height`、`fit`、`background` 等字段。`tab` 只接受 `enhance`、`settings`、`help`、`cooperation`；`position` 只接受 `left`、`right`；`fit` 支持 `natural`、`contain`、`cover`、`fill`。配置加载失败、字段非法、广告位关闭或图片地址为空时，对应广告位隐藏，不影响主功能。

尺寸配置优先使用 `width`，也支持 `width_min`、`width_vw`、`width_max` 组合生成响应式宽度。图片比例由 `fit` 决定：`natural` 保持原图比例并尽量放大；`contain` 保持比例完整显示；`cover` 保持比例并裁切填满；`fill` 强制填满，可能变形。验证方式包括前端构建、UI 检查、Muggle Leads 测试、D1 迁移，以及请求远端广告配置接口。
