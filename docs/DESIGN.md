...
Web UI 在增强、设置、帮助、合作四个 tab 的宽屏左右空白区域提供广告位。每个 tab 左右各一个广告位，共 8 个广告位。前端默认读取 `VITE_AD_CONFIG_URL` 指定的 JSON；未配置时读取 `/ad-slots.json`。广告配置只控制展示，不承担广告后台、点击统计、转化追踪或用户数据保存。

配置文件使用 `slots` 数组描述广告位。每项包含 `tab`、`position`、`enabled`、`image_url`、`click_url`、`alt`、`title`、`width`、`max_height`、`fit`、`background` 等字段。`tab` 只接受 `enhance`、`settings`、`help`、`cooperation`；`position` 只接受 `left`、`right`；`fit` 支持 `natural`、`contain`、`cover`、`fill`。配置加载失败、字段非法、广告位关闭或图片地址为空时，对应广告位隐藏，不影响主功能。

尺寸配置优先使用 `width`，也支持 `width_min`、`width_vw`、`width_max` 组合生成响应式宽度。图片比例由 `fit` 决定：`natural` 保持原图比例并尽量放大；`contain` 保持比例完整显示；`cover` 保持比例并裁切填满；`fill` 强制填满，可能变形。验证方式包括前端构建、UI 检查，以及通过本地服务请求 `/ad-slots.json` 和广告图片资源。
